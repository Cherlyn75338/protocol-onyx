...
contract FeeRoundingTest {
    ...
    function test_deposit_math_rounding_favors_protocol() public {
        // sharePrice=2e18, asset->value conversion assumed exact here.
        uint256 sharePrice = 2e18;
        // User deposits assets worth value=3e18 (1.5 shares exact, but integer methods floor)
        uint256 value = 3e18;
        uint256 grossShares = ValueHelpersLib.calcSharesAmountForValue(sharePrice, value); // floor(1e18*3e18/2e18)=1e18
        require(grossShares == 1e18, "gross shares floors down");

        // entrance fee 10% -> floor(1e18*1000/10000)=1e17
        uint256 feeShares = __calcEntranceExitFee(grossShares, 1000);
        require(feeShares == 1e17, "fee shares floors down");

        uint256 netShares = grossShares - feeShares;
        require(netShares == 9e17, "net shares consistent");
    ...
    function test_redeem_math_rounding_and_burns_gross() public {
        ...
        // exit fee 1% -> floor(5e17*100/10000)=5e13
        uint256 feeShares = __calcEntranceExitFee(grossSharesToRedeem, 100);
        require(feeShares == 5e13, "exit fee shares floors down");

        uint256 netShares = grossSharesToRedeem - feeShares;
        // assets to user computed from net shares
        uint256 valueDue = ValueHelpersLib.calcValueOfSharesAmount(sharePrice, netShares);
        // floor(2e18* (5e17-5e13)/1e18) = approx 1e18 - floor( (2e18*5e13)/1e18 )
        require(valueDue > 0, "user assets non-zero");
    }
}
